<footer>
    <div id="footer-content">
        <div id="infos">
            <h4>Information</h4>

            <p>
                Quisque commodo facilisis purus, interdum volutpat arcu
                viverra sed.
            </p>
            <p>
                <strong>Tel :</strong>
                <a href="tel: +33605040302">06 05 04 03 02</a>
            </p>
            <p>
                <strong>Email :</strong>
                <a href="mailto: support@powerofmemory.com">support@powerofmemory.com</a>
            </p>
            <p>
                <strong>Location :</strong>
                <a href="https://www.google.com/maps/place/Paris" target="_blank">Paris</a>
            </p>

            <div class="social-medias">
                <a href="https://www.facebook.com" target="_blank"><img src="assets/images/facebook.png"
                        alt="Facebook logo" /></a>
                <a href="https://www.twitter.com" target="_blank"><img src="assets/images/twitter.png"
                        alt="Twitter logo" /></a>
                <a href="https://youtu.be/dQw4w9WgXcQ" target="_blank"><img src="assets/images/youtube.png"
                        alt="Youtube logo" /></a>
                <a href="https://www.pinterest.com" target="_blank"><img src="assets/images/pinterest.png"
                        alt="Pinterest logo" /></a>
                <a href="https://www.instagram.com" target="_blank"><img src="assets/images/instagram.png"
                        alt="Instagram logo" /></a>
            </div>
        </div>

        <div id="nav">
            <h4>Power Of Memory</h4>

            <ul>
                <li><a href="register.html">Jouer !</a></li>
                <li><a href="scores.html">Les scores</a></li>
                <li><a href="contact.html">Nous contacter</a></li>
            </ul>
        </div>
    </div>

    <div id="copyrights">
        <p>Copyrights © 2022 Tous droits réservés</p>
    </div>
</footer>